...
def download(url):
    ...
    split_dir = os.path.join(args.decompress_dir, os.path.splitext(download_name)[0])
    if os.path.exists(split_dir):
        print(f'Split exists: {split_dir}. Skipping...')
        return

    if os.path.exists(download_file):
        print(f'File exists: {download_file}. Skipping...')
        return
    else:
...
def has_enough_space(required_space_gb, path="/media/titrom/archive/"):
    """ Check if there is enough disk space available.

    Args:
    required_space_gb (float): Required space in gigabytes.
    path (str): Path to check disk space for, default is root.
...
for url in urls_to_download:
    if has_enough_space(50):
        download(url)
    else:
        print("Not enough disk space to download files.")
        break
